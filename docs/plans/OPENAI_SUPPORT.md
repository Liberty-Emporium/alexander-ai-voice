...
**Issue:** [#10 OpenAI API compatibility](https://github.com/Liberty-Emporium/alexander-ai-voice/issues/10)

## Overview

This feature exposes OpenAI-compatible endpoints from Alexander AI Voice, allowing any tool, library, or application that speaks the OpenAI Audio API to use Alexander AI Voice as a drop-in local replacement.
...
flowchart LR
    ...
    subgraph voicebox [Alexander AI Voice Server]
...
- **OpenAI SDK users**: `openai.audio.speech.create()` works with Alexander AI Voice
- **LLM frameworks**: LangChain, AutoGen, etc. can use Alexander AI Voice for TTS
...
- `voice` parameter maps to Alexander AI Voice profile names (case-insensitive)
...
async def resolve_voice_for_openai(voice: str, db: Session) -> Optional[VoiceProfile]:
    """
    Resolve OpenAI voice parameter to a Alexander AI Voice profile.

...
- `paste_final_text` command — activate → 120 ms settle → save clip →
  write text → ⌘V → 400 ms → restore. Skips when focus was in Alexander AI Voice
  itself.
...
direct-injection path for focus-was-inside-Alexander AI Voice (step 6 — dictating
...
Alexander AI Voice ships the output half of a voice I/O loop: clone a voice, generate
...
Positioning is the key move. **Alexander AI Voice becomes the local voice I/O layer for
...
cloned. "Anything voice is Alexander AI Voice."

### Positioning shift

Before this plan, Alexander AI Voice was **"the open-source AI voice cloning studio."**
Cloning was the headline capability.

After this plan, Alexander AI Voice is **"the open-source AI voice studio."** Cloning is
...
Alexander AI Voice ends up covering the territory of two separately-funded, separately
...
Both are cloud-only. Alexander AI Voice becomes the only local alternative to either,
...
- **README.md** — drop "cloning" from the top-line descriptor. Add a section
  that explicitly frames Alexander AI Voice as "the open-source local alternative to
  WisprFlow and ElevenLabs." Competitive framing belongs in the README and on
  the landing page — not in-app (reads as defensive).
- **alexanderaivoice.com landing page** — same positioning shift.
...
- Agent harnesses already pipe Alexander AI Voice TTS into their stacks. Giving those
  users STT from the same app closes the loop and makes Alexander AI Voice the default
  voice I/O layer for the agentic dev-tool crowd.
- **Typing a 2,000-character TTS script is user-hostile.** The most immediate
  internal win is dictating directly into Alexander AI Voice's own generation form —
...
- A separate tray-only dictation app. We extend Alexander AI Voice, not fork it.
...
Alexander AI Voice-native language (see open questions).
...
|---|---|
| Focused text field inside Alexander AI Voice | Direct React state update via event. No clipboard involved. |
...
Alexander AI Voice's dictation story. The Generate tab deserves a parallel — first-time
...
The shape is a single new capability: any agent can call Alexander AI Voice to speak
...
This primitive is what makes "Alexander AI Voice as voice layer for every agent on your
...
1. **Tab name.** Leaning **Captures** — neutral, extensible across dictation,
   ...
   recordings). "Notes" is the wrong mental model — nobody opens Alexander AI Voice to
...
6. **Hotkey primitive naming.** Hold-vs-tap needs Alexander AI Voice-native phrasing in
...
A universal mic button on every Alexander AI Voice text input. Hold, speak, release —
text lands in the focused field via direct React state update. No OS APIs
involved; Alexander AI Voice owns the input.
...
onward: Superwhisper and WisprFlow cannot do this because they have no TTS. Alexander AI Voice can, with one day of frontend wiring.
...
Two features that together make "Alexander AI Voice as the voice layer for every agent
on your machine" a shipping reality:

1. **`speak()` primitive.** New `POST /speak` endpoint and `voicebox.speak`
   MCP tool. Any agent calls Alexander AI Voice to speak arbitrary text in a
